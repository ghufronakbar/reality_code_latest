...
const productSchema = z.object({
  ...
  demoVideoUrl: z.string().url().optional().or(z.literal("").transform(() => undefined)),
...
export default function NewProductPageClient({ categories }: Props) {
  ...
  const onSubmit = async (values: ProductForm) => {
    try {
      ...
      const data = (await res.json()) as Api<Product>;
      if (res.ok) {
        toast.success("Product created!");
        router.push(`/shop/products/${data.data.id}`);
      } else {
  ...
  // --- PreviewProduct (inline, similar to edit page) ---
  const PreviewProduct = ({ product }: { product: ProductForm }) => (
      ...
      <CardContent className="my-4">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <ProductInfo
            product={{
              category: {
                name: categories.find((c) => c.id === product.productCategoryId)?.name || "",
                description: "",
              ...
              productReviews: [],
              productSpesifications:
                product.productSpesifications?.map((item: { name: string; value: string }) => ({
                  id: "",
                  name: item.name,
                  productId: "",
                  createdAt: new Date(),
              ...
              productTools:
                product.productTools?.map((item: { name: string; value: string; description?: string }) => ({
                  id: "",
                  name: item.name,
                  productId: "",
                  createdAt: new Date(),
    ...
    <div className="container mx-auto py-10 px-2">
      <Tabs defaultValue="edit" className="w-full">
        ...
        <TabsContent value="edit">
          {/* --- FORM CODE START --- */}
          {/* (move the entire form code here, unchanged) */}
          {/* Basic Info */}
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {imagesPreview.map((img, i) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          className="absolute -top-2 -right-2 bg-destructive text-white rounded-full p-1 text-xs opacity-80 hover:opacity-100 transition"
            ...
            <Card className="bg-background/80">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    <Label htmlFor="productCategoryId">License</Label>
                    <Controller
                      control={form.control}
                      name="license"
                      render={({ field }) => (
                    ...
                    <div className="text-sm flex flex-row items-end mt-4 gap-2">
                      <p>Not sure about the license you want to choose? </p>
                      <Link
                        href="/about-license"
                        target="_blank"
        ...
        <TabsContent value="preview">
          <PreviewProduct product={watchAll} />
        </TabsContent>
      </Tabs>
    </div>
  );
